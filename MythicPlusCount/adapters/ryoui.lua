-- MythicPlusCount - RyoUI Adapter
-- FontString overlay for RyoUI nameplates.
-- RyoUI stores health bar at nameplate.healthbar (StatusBar).
local ADDON_NAME, NS = ...
local MPC = NS.MPC

local RyoUI = {}
MPC.Adapters = MPC.Adapters or {}
MPC.Adapters.RyoUI = RyoUI

local overlays = {}

function RyoUI:IsAvailable() return _G.RyoUI ~= nil end
function RyoUI:Init() MPC:Debug("RyoUI adapter initialized") end

function RyoUI:OnNameplateAdded(unitToken)
...
function RyoUI:OnNameplateRemoved(unitToken)
    local nameplate = C_NamePlate.GetNamePlateForUnit(unitToken)
    if nameplate then self:HideOverlay(nameplate) end
end

function RyoUI:GetOrCreateOverlay(nameplate)
...
function RyoUI:ShowOverlay(nameplate, info)
    ...
    -- RyoUI uses frame.healthbar (StatusBar)
    local anchor = nameplate
    if nameplate.healthbar then
        anchor = nameplate.healthbar
...
function RyoUI:HideOverlay(nameplate)
